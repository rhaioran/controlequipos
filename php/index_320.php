...
unidad.codigolocacion =  locacion.codigolocacion ORDER BY direcciones.nombredireccion ASC";
...
<?php
include("coneccion_bd.php");
$sql="SELECT codloc,locacion
FROM
locacion_administrativa
";
  $result=mysqli_query($conexion,$sql);  
  $options2="";  
  while ($row=mysqli_fetch_array($result)) 
  {  
  	$id		= $row["codloc"];
  	$name	= $row["locacion"];
  	$options2.='<option value='.$id.'>'.utf8_encode($name).'</option>';
  }
?>

<?php
include("coneccion_bd.php");
$sql="SELECT codunid,unidad
FROM
...
";
  ...
  {  
  	...
  	$options4.='<option value='.$id.'>'.utf8_encode($name).'</option>';
  }
?>

<?php
include("coneccion_bd.php");
$sql="SELECT coddirec,nombre
FROM
...
";
  ...
  {  
  	$id		= $row["coddirec"];
  	$name	= $row["nombre"];
  	$options3.='<option value='.$id.'>'.utf8_encode($name).'</option>';
  }
?>
...
window.open(url, "index_321.php",'width='+ancho+',height='+alto+',menubar=0,toolbar=0,directories=0,scrollbars=no,resizable=no,left='+posicion_x+',top='+posicion_y+'');
}
</script>
...
posicion_x=(screen.width/2)-(ancho/2); 
posicion_y=(screen.height/2)-(alto/2); 
window.open(url, "index_322.php",'width='+ancho+',height='+alto+',menubar=0,toolbar=0,directories=0,scrollbars=no,resizable=no,left='+posicion_x+',top='+posicion_y+'');
}
</script>
...
posicion_x=(screen.width/2)-(ancho/2); 
posicion_y=(screen.height/2)-(alto/2); 
window.open(url, "index_323.php",'width='+ancho+',height='+alto+',menubar=0,toolbar=0,directories=0,scrollbars=no,resizable=no,left='+posicion_x+',top='+posicion_y+'');
}
</script>    
                ...
                <div class="">
                    <div class="grid simple">
                        ...
                        <div class="grid-body no-border">
                            <form action="guardar_nueva_unidad.php" method="post" id="form_traditional_validation">  
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-with-icon  right">                                       
                                        <i class=""></i>
                                        	<select name="locacion" id="locacion" class="form-control" required="">
                                    			<option value="">Seleccione <?php echo $options2?>
                                  			</select>
                                            <div><a href="javascript:popup4('index_323.php',700,400)">Añadir otra +</a></div>                               
                                    </div>
                                </div>                            
                                <div class="form-group">
                                    ...
                                    <div class="input-with-icon  right">                                       
                                        <i class=""></i>
                                        	<select name="unidad" id="unidad" class="form-control" required="">
                                    			<option value="">Seleccione <?php echo $options4?>
                                  			</select>
                                        <div><a href="javascript:popup3('index_322.php',700,350)">Añadir otra +</a></div>
                                    </div>
                                </div>                            
                                    ...
                                    <div class="input-with-icon  right">                                       
                                        <i class=""></i>
                                        	<select name="direccion" id="direccion" class="form-control" required="">
                                    			<option value="">Seleccione <?php echo $options3?>
                                  			</select>
                                        <div><a href="javascript:popup2('index_321.php',700,350)">Añadir otra +</a></div>                               
                                    </div>
                                </div>
