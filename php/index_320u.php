...
{
  $codigounidad =$dato2['codigounidad'];
...
<?php
include("coneccion_bd.php");
$sql="SELECT codigolocacion,nombrelocacion
FROM
locacion
WHERE
codigolocacion = '$codigolocacion'";
  $result=mysqli_query($conexion,$sql);  
  $options2="";  
  while ($row=mysqli_fetch_array($result)) 
  {  
  	$id		= $row["codigolocacion"];
  	$name	= $row["nombrelocacion"];
...
<?php
include("coneccion_bd.php");
$sql="SELECT codigounidad,nombreunidad
FROM
unidad
WHERE
